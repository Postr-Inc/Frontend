...
export default function User(props: {
  ...
  useEffect(() => {
    ...
    if(api.cacehStore.get(`user-feed-posts-${page}-${user.id}`)){
      setIsFetching(false);
      let cache = JSON.parse(api.cacehStore.get(`user-feed-posts-${page}-${user.id}`))
      ...
      .list({
        ...
        expand: ["author", "comments.user", "comments.likes", "comments"],
        page:  1,
        sort: `-created`,
      })
      .then((e: any) => {
        ...
        setHasMore(true);
        setIsFetching(false);
        if(!api.cacehStore.has(`user-feed-posts-${page}-${user.id}`)){ api.cacehStore.set(`user-feed-posts-${page}-${user.id}`, {items:e.items, totalItems:e.totalItems, totalPages: e.totalPages}, 1200)}
      });
    }
  ...
  function swapFeed(pageValue: string, pg: number = 0) {
    if(api.cacehStore.has(`user-feed-${pageValue}-${page}-${user.id}`)){
      setIsFetching(false);
      let cache = JSON.parse(api.cacehStore.get(`user-feed-${pageValue}-${page}-${user.id}`))
      setArray(cache.value.items);
      setTotalPages(cache.value.totalPages);
      setTotalItems(cache.value.totalItems);
      return
    ...
    pageValue === "posts" || pageValue === "likes"
      ...
      .then((e: any) => {
        if(!api.cacehStore.has(`user-feed-${page}-${user.id}`)){ api.cacehStore.set(`user-feed-${pageValue}-${page}-${user.id}`, {items:e.items, totalItems:e.totalItems, totalPages: e.totalPages}, 230)}
            ...
            {isFetching && feedPage !== "media"
              ...
              ? array.map((e: any, index: number) => {
                  return (
                    ...
                    >
                      <Post
                        cache={api.cacehStore.get(`user-feed-${feedPage}-${page}-${user.id}`)}
                        {...e}
              ...
              ?  array.length > 0 ? array.map((e: any) => {
                ...
                return (
                  <div className="mt-5 xl:mt-0   mb-6 xl:p-3 lg:p-3 md:p-3 md:mt-0 lg:mt-0"  >
                  <div className="flex   justify-between">
                    <div className="flex flex-row  gap-2   ">
                      <img
                        ...
                        alt="profile"
                        className="rounded  w-12 h-12 cursor-pointer"
                      ></img>
