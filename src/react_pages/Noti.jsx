...
export default function Noti() {
  ...
  notifications = notifications.filter((noti, index, self) => {
    return self.findIndex((n) => {
      ...
      }else if(n.expand  && noti.expand  && n.expand.author && noti.expand.author){
        return n.expand.author && n.expand.author.id === noti.expand.author.id && n.type === noti.type
      }
...
</svg> : noti.type === 'comment_like' ? 
<svg xmlns="http://www.w3.org/2000/svg" fill="#F13B38" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#F13B38" className="w-6 h-6">
  <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
</svg> : noti.type === 'follow' ?
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="text-sky-500 w-6 h-6">
  <path strokeLinecap="round" strokeLinejoin="round" d="M19 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 0110.374 21c-2.331 0-4.512-.645-6.374-1.766z" />
              ...
              
             {
               ...
               || noti.type === "comment_like" ? <span className="text-sm">{noti.title}</span>  : noti.type === "follow" ? <span className="text-sm">followed you</span> : ""
             }
  ...
  <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
              ...
              ref={(el)=>{
                if(el && noti.expand && noti.expand.post){
                  el.innerHTML = noti.expand.post.content || noti.notification_body
                }
              }}
              ></span>
             </div>
